...
def create_pdf(form_data, file_path="invoice.pdf"):
    ...
    # Company Header (Placeholder - will be replaced by uploaded template)
    ...
    # Determine if Invoice or Quotation based on form data
    ...
    if is_invoice:
        ...
        for i, item in enumerate(form_data.get('items', [])):
            ...
            print(f"Debug: quantity_val={quantity_val}, type={type(quantity_val)}")
            print(f"Debug: rate_val={rate_val}, type={type(rate_val)}")
        ...
        table_header = ["SNo", "Description", "Price"]
        table_data = [table_header]
        for i, item in enumerate(form_data.get('items', [])):
            table_data.append([
                str(i + 1),
                item.get('description', ''),
                f"Rs {float(item.get('rate', 0) or 0):.2f} / piece"
            ])
        col_widths = [0.5*inch, 5*inch, 2*inch]
    ...
    if is_invoice:
        ...
        # This table is aligned to the right side of the page
    ...
    # Create a table to position bank details correctly
